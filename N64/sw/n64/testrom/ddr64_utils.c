...
void verify_memory_range(uint32_t base, uint32_t offset, uint32_t len)
{
	...
	case DDR64_RAND_ADDRESS_START:
		assert(start >= DDR64_RAND_ADDRESS_START);
		assert(start <= DDR64_RAND_ADDRESS_END);
		assert(end >= DDR64_RAND_ADDRESS_START);
		assert(end <= DDR64_RAND_ADDRESS_END);
		break;
