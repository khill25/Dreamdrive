...
void init_cs0_low_program() {
	...
	pio_sm_set_pindirs_with_mask(pio1, sm, 0x800001F, 0x8000000);
...
void init_cs1_low_program() {
	uint sm = 1;
	uint offset = pio_add_program(pio1, &cs1_low_program);
	pio_sm_config c = cs1_low_program_get_default_config(offset);
...
void init_bus_read_request_program() {
	uint sm = 2;
	uint offset = pio_add_program(pio1, &bus_read_request_program);
	pio_sm_config c = bus_read_request_program_get_default_config(offset);

	// 0-15 output, 16,17 input 	27 output
	pio_sm_set_pindirs_with_mask(pio1, sm, 0x803FFFF, 0x803FFFF);
...
void init_bus_write_request_program() {
	uint sm = 3;
	uint offset = pio_add_program(pio1, &bus_write_request_program);
	pio_sm_config c = bus_write_request_program_get_default_config(offset);

	// 0-15 input, 16,17 input 	27 output
	pio_sm_set_pindirs_with_mask(pio1, sm, 0x803FFFF, 0x8030000);
