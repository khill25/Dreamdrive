...
uint8_t rx_interconnect_fetch_byte() {
    // 16-bit read from the uppermost byte of the FIFO, as data is left-justified
    io_rw_8 *rxfifo_shift = (io_rw_8*)&rx_interconnect_config.pio->rxf[rx_interconnect_config.sm] + 3;
    while (pio_sm_is_rx_fifo_empty(rx_interconnect_config.pio, rx_interconnect_config.sm))
        tight_loop_contents();
...
void rx_interconnect_interrupt() {
    while(interconnect_rx_has_data()) {
        ...
        rxRingBuffer.buf[rxRingBuffer.head++] = rx_interconnect_fetch_byte();
...
void interconnect_tx(uint8_t* buf, int len) {
    ...
    // }
    for(int i = 0; i < len; i++) {
        pio_sm_put_blocking(tx_interconnect_config.pio, tx_interconnect_config.sm, buf[i]);
    }
}
...
static inline void interconnect_tx_program_init(PIO pio, uint sm, uint offset, uint pin_tx_start, uint pin_ctrl, uint divider) {
    ...
    pio_sm_set_pins_with_mask(pio, sm, 0x00000000, 0xD400000);
    pio_sm_set_pindirs_with_mask(pio, sm, 0xD000000, 0xD400000);
...
static inline void interconnect_rx_program_init(PIO pio, uint sm, uint offset, uint pin_rx_start, uint pin_ctrl, uint divider) {
    ...
    pio_sm_set_pins_with_mask(pio, sm, 0x00000000, 0xD400000);
    pio_sm_set_pindirs_with_mask(pio, sm, 0x00400000, 0xD400000);

    pio_sm_set_consecutive_pindirs(pio, sm, pin_rx_start, NUM_INTERCONNECT_DAT_RX_PINS, false);
